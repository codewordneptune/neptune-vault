...
impl ProverJob {
    ...
    async fn prove(&self, rx: JobCancelReceiver) -> JobCompletion {
        ...
        // job queue in the browser. Proving goes through vault-prover, which
        // calls Triton VM directly. This path must never be reached.
    ...
    fn prove_unsupported_on_wasm(
        ...
        unimplemented!(
            "ProverJob::prove is not available on wasm32; use vault-prover to prove in-process"
        )
    }
