...
impl ProverJob {
    ...
    async fn prove(&self, rx: JobCancelReceiver) -> JobCompletion {
        ...
        // wasm32 (Neptune Vault): there is no external prover process and no
        // job queue in the browser. Proving goes through vault-prover-legacy,
        // which calls Triton VM directly. This path must never be reached.
        #[cfg(all(not(any(test, feature = "test-helpers")), target_arch = "wasm32"))]
        let result = self.prove_unsupported_on_wasm(rx);

        job_completion_from_prover_result(result)
    }

    #[cfg(target_arch = "wasm32")]
    fn prove_unsupported_on_wasm(
        &self,
        _rx: JobCancelReceiver,
    ) -> Result<ProverProcessCompletion, VmProcessError> {
        unimplemented!(
            "ProverJob::prove is not available on wasm32; use vault-prover-legacy to prove in-process"
        )
    }
