...
// compiled to wasm: these tests read back what it wrote.

function utxo(hash: string, height: number, amount: string, own: number | null = null): StoredUtxo {
  return coin(hash, height, { own_build_height: own, amount_nau: `${amount}000000000000000000000000000000`, amount });
}
...
/** Stands in for the core's scan, which needs real blocks and keys. */
class FakeCore {
  ...
  nextKeyIndexAfter = 1;
  /** The coin index is keyed by absolute index sets; the fake node keys it by coin hash. */
  async absoluteIndexSets() {
    return JSON.stringify(await vault.store.ledger('acc', { op: 'unspentHashes' }));
  }
  async scanBlocks(op: Extract<LedgerOp, { op: 'scanBlocks' }>): Promise<ScanResult> {
    const blocks = (JSON.parse(op.blocksResponse) as { result: { blocks: unknown[] } }).result.blocks;
    // What the engine hands the real scan to watch for: the pending sends' outputs.
    const expectation: ScanExpectation = { from: op.from, to: op.to, prev_hash: op.prevHash, watch: await vault.store.ledger('acc', { op: 'watchedCommitments' }) };
...
async function setup(options: { rebuild?: boolean } = {}) {
  ...
  vault = await testEngine({
    scanBlocks: (op) => core.scanBlocks(op),
    announcementFlags: () => '[]',
    absoluteIndexSets: () => core.absoluteIndexSets(),
  });
  vault.unlock();
...
describe('sync engine', () => {
  it('scans from the birthday in batches and records incoming funds', async () => {
    ...
    const utxos = await view.getAllFromIndex('utxos', 'byAccount', 'acc');
    expect(utxos).toHaveLength(1);
    expect(utxos[0].amount).toBe('2');
    expect(utxos[0].spentHeight).toBeNull();
    const history = await view.getAllFromIndex('history', 'byAccount', 'acc');
    expect(history.map((h) => [h.kind, h.status, h.height])).toEqual([['received', 'confirmed', 5]]);
    expect((await view.get('accounts', 'acc'))?.nextKeyIndices).toEqual({ generation: 2, ec_hybrid: 0, viewing: 0 });
    expect((await view.get('syncState', 'acc'))?.syncedHash).toBe('hash-10');
  });
  ...
  it('marks spends and confirms the pending send that reserved the input', async () => {
    ...
    await view.put('history', { key: 'acc:sent:tx-1', accountId: 'acc', kind: 'sent', status: 'pending', txid: 'tx-1', amountNau: '1', feeNau: '0', timestampMs: 0, height: null, inputHashes: ['u1'], recipient: 'r', error: null });

    node.extendTo(8);
    core.spent.set(8, ['u1']);
    await engine.syncOnce();
    const spent = (await view.get('utxos', 'acc:u1'))!;
  ...
  it('confirms a send by its own outputs, and fails it when its coins were spent by another transaction', async () => {
    ...
    await engine.syncOnce();
    const reserve = async (hash: string, txid: string) => view.put('utxos', { ...(await view.get('utxos', `acc:${hash}`))!, pendingTxid: txid });
    const pending = (txid: string, inputs: string[], commitment: string) =>
      view.put('history', { key: `acc:sent:${txid}`, accountId: 'acc', kind: 'sent', status: 'pending', txid, amountNau: '1', feeNau: '0', timestampMs: 0, height: null, inputHashes: inputs, recipient: 'r', error: null, outputs: [{ commitment, role: 'recipient' }] });
    ...
    expect((await view.get('history', 'acc:sent:tx-mine'))?.status).toBe('confirmed');
    const lost = (await view.get('history', 'acc:sent:tx-lost'))!;
    ...
    expect(await view.get('utxos', 'acc:u2')).toMatchObject({ spentHeight: 8, spentTxid: null, pendingTxid: null });
    expect(await view.get('utxos', 'acc:u3')).toMatchObject({ spentHeight: null, pendingTxid: null });
    // And the spend shows as one made elsewhere.
    expect((await view.get('history', 'acc:spent:8'))?.inputHashes).toEqual(['u2']);
  });
  ...
  it('rolls back when the next block does not follow the last one scanned, though the node called it canonical', async () => {
    ...
    expect(result.phase).toBe('done');
    expect(result.syncedHeight).toBe(10);
    expect(await view.get('utxos', 'acc:orphaned')).toBeUndefined();
    const blocks = await view.getAllFromIndex('blocks', 'byAccountHeight', IDBKeyRange.bound(['acc', 0], ['acc', Infinity]));
  ...
  it('records a spend made elsewhere as a send, with what came back as change', async () => {
    ...
    await engine.syncOnce();
    const sent = (await view.get('history', 'acc:spent:8'))!;
    ...
    expect(sent.outputs?.map((o: { commitment: string }) => o.commitment)).toEqual([]);
    expect(sent.inputHashes).toEqual(['u1']);

    // A rewind below the block drops the row again.
    await engine.rollBack(7, null);
    expect(await view.get('history', 'acc:spent:8')).toBeUndefined();
  });
  ...
  it('rolls back to the last canonical block after a reorg and rescans', async () => {
    ...
    core.incoming.set(7, [utxo('orphaned', 7, '1')]);
    await engine.syncOnce();
    expect(await view.get('utxos', 'acc:orphaned')).toBeDefined();
    ...
    expect(await view.get('utxos', 'acc:orphaned')).toBeUndefined();
    expect(await view.get('history', 'acc:recv:orphaned')).toBeUndefined();
    expect(await view.get('utxos', 'acc:fresh')).toBeDefined();
    expect((await view.get('syncState', 'acc'))?.syncedHash).toBe('fork-9');
    const blocks = await view.getAllFromIndex('blocks', 'byAccountHeight', IDBKeyRange.bound(['acc', 0], ['acc', Infinity]));
  ...
  it('sets an unknown start height to the tip at first sync', async () => {
    ...
    await view.put('accounts', { ...(await view.get('accounts', 'acc'))!, birthdayHeight: 0 });
    node.extendTo(9);
    await engine.syncOnce();
    expect((await view.get('accounts', 'acc'))?.birthdayHeight).toBe(9);
    expect(node.getBlocksCalls).toEqual([[9, 9]]);
  });

  it('clamps a start height above the tip to the tip', async () => {
    const { node, engine } = await setup();
    await view.put('accounts', { ...(await view.get('accounts', 'acc'))!, birthdayHeight: 500 });
    node.extendTo(9);
    const result = await engine.syncOnce();
    expect((await view.get('accounts', 'acc'))?.birthdayHeight).toBe(9);
    expect(result.syncedHeight).toBe(9);
  ...
  it('fast restore scans the flagged blocks, then the blocks that spent what it found', async () => {
    const { node, core, engine } = await setup();
    await view.put('accounts', { ...account, birthdayHeight: 0, restore: 'fast' });
    node.extendTo(20);
    ...
    expect((await view.get('utxos', 'acc:a'))?.spentHeight).toBe(15);
    expect((await view.get('utxos', 'acc:b'))?.spentHeight).toBeNull();
    expect((await view.get('syncState', 'acc'))?.syncedHeight).toBe(20);
    expect((await view.get('syncState', 'acc'))?.syncedHash).toBe('hash-20');
  ...
  it('fast restore looks again at a block that spent a coin it only found later', async () => {
    ...
    expect((await view.get('utxos', 'acc:x'))?.spentHeight).toBe(15);
    expect((await view.get('utxos', 'acc:change'))?.spentHeight).toBeNull();
  });
  ...
  it('a block written again keeps what is known about its coins, and a coin written afresh is held again by the send that spends it', async () => {
    ...
    await engine.syncOnce();
    expect((await view.get('utxos', 'acc:early'))?.spentHeight).toBe(6);
    await view.put('utxos', { ...(await view.get('utxos', 'acc:late'))!, pendingTxid: 'tx-p' });
    await view.put('history', { key: 'acc:sent:tx-p', accountId: 'acc', kind: 'sent', status: 'pending', txid: 'tx-p', amountNau: '1', feeNau: '0', timestampMs: 0, height: null, inputHashes: ['late'], recipient: 'r', error: null });

    // A reorganisation above block 6 removes coin "late"; the new chain has it again in block 7.
    node.forkAbove(6, 9);
    await engine.syncOnce();
    expect((await view.get('utxos', 'acc:late'))?.pendingTxid).toBe('tx-p');
    expect((await view.get('utxos', 'acc:early'))?.spentHeight).toBe(6);
  });
  ...
  it('never lowers the start height or drops the local view on a node\'s say-so', async () => {
    ...
    stale.extendTo(2);
    const low = await new SyncEngine(db, stale as unknown as NodeClient, vault.store as unknown as WalletCore, 'acc', { batchSize: 4, keepBlocks: 100 }).syncOnce();
    ...
    const lost = await new SyncEngine(db, stranger as unknown as NodeClient, vault.store as unknown as WalletCore, 'acc', { batchSize: 4, keepBlocks: 100 }).syncOnce();
    expect(lost.phase).toBe('error');
    expect(lost.message).toMatch(/None of the blocks/);
    expect(await view.get('utxos', 'acc:u1')).toBeDefined();
    expect((await view.get('syncState', 'acc'))?.syncedHeight).toBe(10);
  });
  ...
  it('stops after the batch in flight, leaving that batch unwritten', async () => {
    ...
    expect((await view.get('syncState', 'acc'))?.syncedHeight).toBe(6);
  ...
  it('reports node errors without corrupting state', async () => {
    ...
    expect(await view.get('syncState', 'acc')).toBeUndefined();
  });
});
