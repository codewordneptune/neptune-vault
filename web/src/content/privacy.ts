...
export const PRIVACY: PrivacySection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      'The node you chose in Settings. The app talks only to that node, over its JSON-RPC interface, and only for four things: fetching blocks to scan, asking which transactions are waiting in the mempool and fetching the ones it has not seen, asking for membership proofs of the coins it is about to spend, and submitting a transaction you sent. The node sees your network address and everything you ask it. From what you ask, it can learn which blocks you scan from (roughly when your wallet was created), which coins you own (the membership-proof request names them), and the transactions you send. It does not receive your phrase, your password, your addresses as such, your contacts, or the notes and names in payment links.',
